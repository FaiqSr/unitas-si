...
<body class="bg-white">
    ...
    <div class="ml-64 p-8">
        ...
        <div class="bg-white p-8 rounded-lg shadow-md border border-gray-200">
            <form action="/dashboard/merchandise/put" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf

                <div>
                    <label for="title" class="block text-sm font-medium text-gray-700">Nama</label>
                    <input type="text" id="title" name="title" value="{{$merch->nama}}" class="w-full mt-1 p-2 border border-gray-300 rounded-md" placeholder="Enter merchandise name" required>
                </div>

                <div>
                    <label for="harga" class="block text-sm font-medium text-gray-700">Harga</label>
                    <input type="number" id="harga" name="harga" value="{{$merch->harga}}" class="w-full mt-1 p-2 border border-gray-300 rounded-md" min="0" placeholder="Enter price" required>
                </div>

                <div>
                    <label for="stok" class="block text-sm font-medium text-gray-700">Stok</label>
                    <input type="number" id="stok" name="stok" value="{{$merch->stok}}" class="w-full mt-1 p-2 border border-gray-300 rounded-md" min="0" placeholder="Enter stock quantity" required>
                </div>

                <div>
                    <label for="body" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                    <textarea id="body" name="body" rows="4" class="w-full mt-1 p-2 border border-gray-300 rounded-md" placeholder="Enter description" required>{{$merch->body}}</textarea>
                </div>
